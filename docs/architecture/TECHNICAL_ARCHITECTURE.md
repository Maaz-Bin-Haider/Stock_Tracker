...
| M9 | Offline/local production use: run the stack in production mode on a single local machine / office LAN — no cloud dependency. Production process managers (gunicorn + `next build`/`next start`), persistent local storage for DB/media/exports (named Docker volumes), `restart: unless-stopped` so the stack survives reboots, LAN access (`ALLOWED_HOSTS` + nginx bind), scheduled local `pg_dump` backups with a documented + tested restore, and simple start/stop/backup make targets for non-technical operators | Stack runs with `DEBUG=False` on the local machine/LAN, survives a reboot, real users complete the full daily flow, uploads/exports persist across restarts, and a scheduled local backup with a verified restore is in place |
