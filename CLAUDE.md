...
The SwissTech Stock Tracker is a web-based inventory system replacing a spreadsheet workflow (`data/source/stock_tracker_original.xlsx`). Django backend under `src/backend` (11 apps under `apps/`, settings split in `config/settings/`), Next.js frontend under `src/frontend`, Docker Compose + nginx under `deployment/`. Phases M0–M7 and manual Mac testing are complete. **M9 is a fresh three-month, single-Admin trial on a different Windows machine.** The Windows target must use a new Git clone and new Docker volumes; never transfer Mac test data, media, or backups. `setup-windows.ps1` automatically generates the gitignored `.env.prod` with random machine-specific secrets. Windows tooling lives in `scripts/{setup-windows.ps1,open-stock-tracker-windows.cmd,install-desktop-launcher-windows.ps1,backup-windows.ps1,restore-windows.ps1}`. It provides a one-click Desktop shortcut and hardened timestamp-matched PostgreSQL/uploaded-media backups every 12 hours with 120-day retention. Root `LOCAL_SETUP_GUIDE.md` is authoritative. The Mac `.env.prod` remains test-only; future M8 server/AWS deployment will use separate production secrets and remains deferred until the Windows trial finishes successfully. Tests run against Dockerized PostgreSQL (`make test`; `config.settings.test`).
